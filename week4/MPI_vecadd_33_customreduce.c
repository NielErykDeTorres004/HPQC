#include <stdio.h>
#include <stdlib.h>
#include <mpi.h>

// function declarations
int check_args(int argc, char **argv);
long long expected_sum(int N);

// custom reduce operation (sum)
void my_sum_op(void *invec, void *inoutvec, int *len, MPI_Datatype *datatype);

int main(int argc, char **argv)
{
    // declare and initialise error handling variable
    int ierror = 0;

    // declare and initialise rank and size variables
    int my_rank, uni_size;
    my_rank = uni_size = 0;

    // declare vector size
    int N = 0;

    // initialise MPI
    ierror = MPI_Init(&argc, &argv);

    // get rank and communicator size
    ierror = MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
    ierror = MPI_Comm_size(MPI_COMM_WORLD, &uni_size);

    // root reads vector size
    if (0 == my_rank)
        N = check_args(argc, argv);

    // broadcast vector size to all ranks
    MPI_Bcast(&N, 1, MPI_INT, 0, MPI_COMM_WORLD);

    // calculate base chunk size and remainder
    int base = N / uni_size;
    int rem = N % uni_size;

    // determine local vector size
    int local_n = base + (my_rank < rem ? 1 : 0);

    // declare root-only variables
    int *sendcounts = NULL;
    int *displs = NULL;
    int *vec = NULL;

    if (0 == my_rank)
    {
        // allocate arrays for scatter
        sendcounts = (int*)malloc(uni_size * sizeof(int));
        displs = (int*)malloc(uni_size * sizeof(int));

        // calculate displacements
        int offset = 0;
        for (int r = 0; r < uni_size; r++)
        {
            sendcounts[r] = base + (r < rem ? 1 : 0);
            displs[r] = offset;
            offset += sendcounts[r];
        } // end for

        // allocate and initialise vector
        vec = (int*)malloc(N * sizeof(int));
        for (int i = 0; i < N; i++)
            vec[i] = i % 100;
    } // end if (0 == my_rank)

    // allocate local vector
    int *local_vec = (int*)malloc(local_n * sizeof(int));

    // scatter vector to all ranks
    MPI_Scatterv(vec, sendcounts, displs, MPI_INT,
                 local_vec, local_n, MPI_INT,
                 0, MPI_COMM_WORLD);

    // compute local sum
    long long local_sum = 0;
    for (int i = 0; i < local_n; i++)
        local_sum += local_vec[i];

    // create custom MPI operation
    MPI_Op my_op;
    MPI_Op_create(my_sum_op, 1, &my_op);

    // ---------- built-in reduce (MPI_SUM) ----------
    MPI_Barrier(MPI_COMM_WORLD);
    double start_mpi = MPI_Wtime();

    long long global_sum_mpi = 0;
    MPI_Reduce(&local_sum, &global_sum_mpi, 1, MPI_LONG_LONG, MPI_SUM, 0, MPI_COMM_WORLD);

    double end_mpi = MPI_Wtime();
    double time_mpi = end_mpi - start_mpi;

    // ---------- custom reduce (my_op) ----------
    MPI_Barrier(MPI_COMM_WORLD);
    double start_custom = MPI_Wtime();

    long long global_sum_custom = 0;
    MPI_Reduce(&local_sum, &global_sum_custom, 1, MPI_LONG_LONG, my_op, 0, MPI_COMM_WORLD);

    double end_custom = MPI_Wtime();
    double time_custom = end_custom - start_custom;

    // root prints correctness and timing
    if (0 == my_rank)
    {
        long long exp = expected_sum(N);

        printf("MPI_SUM result     : %lld (expected %lld)\n", global_sum_mpi, exp);
        printf("Custom op result   : %lld (expected %lld)\n", global_sum_custom, exp);

        if (global_sum_mpi == global_sum_custom)
            printf("Check: MPI_SUM and Custom op MATCH\n");
        else
            printf("Check: MPI_SUM and Custom op DO NOT MATCH\n");

        printf("Time MPI_SUM       : %.9f s\n", time_mpi);
        printf("Time Custom op     : %.9f s\n", time_custom);
    }

    // free custom op
    MPI_Op_free(&my_op);

    // free memory
    free(local_vec);

    if (0 == my_rank)
    {
        free(vec);
        free(sendcounts);
        free(displs);
    }

    // finalise MPI
    ierror = MPI_Finalize();
    return 0;
}

int check_args(int argc, char **argv)
{
    if (argc != 2)
    {
        fprintf(stderr, "Usage: %s [VECTOR_SIZE]\n", argv[0]);
        exit(-1);
    }
    return atoi(argv[1]);
}

long long expected_sum(int N)
{
    long long sum = 0;
    for (int i = 0; i < N; i++)
        sum += (i % 100);
    return sum;
}

void my_sum_op(void *invec, void *inoutvec, int *len, MPI_Datatype *datatype)
{
    // cast input pointers to the correct datatype
    long long *in = (long long*)invec;
    long long *inout = (long long*)inoutvec;

    // sum each element
    for (int i = 0; i < *len; i++)
        inout[i] = inout[i] + in[i];
}
