...
int main(int argc, char **argv)
{
    ...
    // manual send/recv of partial sums
    if (0 == my_rank)
    {
        // allocate array for partial sums
        long long *all_sums = (long long*)malloc(uni_size * sizeof(long long));

        // store root contribution
        all_sums[0] = local_sum;

        // receive sums from other ranks
        MPI_Status status;
        for (int r = 1; r < uni_size; r++)
            MPI_Recv(&all_sums[r], 1, MPI_LONG_LONG, r, 0, MPI_COMM_WORLD, &status);

        // sum results
        for (int r = 0; r < uni_size; r++)
            global_sum += all_sums[r];
